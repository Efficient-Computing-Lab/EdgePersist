...
edgestorage_endpoint="http://$(sudo k3s kubectl describe pod edstorage-0 --namespace edgestorage | grep Node: | cut -d'/' -f 2):9011"
edge_registry_endpoint="$(sudo k3s kubectl describe pod edstorage-0 --namespace edgestorage | grep Node: | cut -d'/' -f 2)"
sed -e 's?{{EDGE_ENDPOINT}}?"'$edgestorage_endpoint'"?g' "./deployment.yaml" > ./deploymentEdited.yaml
...
k3s kubectl wait --for=condition=ready pods --all -n edgestorage --timeout=40s
